    ...
    : QMainWindow(parent)
{
    ...
    onRefreshPorts();
    appendLog("UI ready. Serial tester is enabled.");
}
...
void MainWindow::buildUi()
{
    ...
    baudBox_->addItems({"9600", "19200", "38400", "57600", "115200"});
    baudBox_->setCurrentText("9600");
    ...
    statusBar()->showMessage("Serial test mode: open COM and send simple commands.");
}
...
void MainWindow::sendCommand(const QString &cmd)
{
    ...
    {
        appendLog(QString("TX skipped (port closed): %1").arg(cmd));
        return;
    }
    ...
    serial_->write(data);
    appendLog(QString("TX -> %1").arg(QString::fromUtf8(data).trimmed()));
}
...
void MainWindow::onTogglePort()
{
    ...
    serial_->setPortName(portBox_->currentText());
    ...
    {
        appendLog(QString("Open failed: %1").arg(serial_->errorString()));
    }
}
...
void MainWindow::onSerialReadyRead()
{
    ...
    if (!rx.isEmpty())
    {
        appendLog(QString("RX <- %1").arg(QString::fromUtf8(rx).trimmed()));
    }
}
