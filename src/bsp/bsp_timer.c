...
/* =========================================================
   ...
   ---------------------------------------------------------
   - 你当前阶段可能只用 tick；后面又要加 buzzer、keypad、soft_timer…
   - 如果ISR写死调用顺序，你每加一个模块都要改ISR文件，维护很糟糕
   - 用hook注册机制：模块自己注册“我需要每1ms被调用”，Timer统一分发
   数据流如下：
     模块Init() -> Timer0_1ms_RegisterHook(模块的Task1ms)
     Timer0_ISR 每1ms触发 -> 依次调用所有已注册hook
   ========================================================= */

/* 允许注册的hook数量：够用就行（51 RAM 紧张） */
#define T0_MAX_HOOKS  6

static t0_hook_t s_hooks[T0_MAX_HOOKS];
static unsigned char s_hook_cnt = 0;
